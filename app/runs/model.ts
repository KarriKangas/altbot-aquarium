...
export type BenchmarkBot = { guid: number; name: string; raceId: number; classId: number; level: number; firstLoginAt: number; retiredAt: number; questsCompletedAmount?: number };
...
export type GroupStat = { label: string; bots: BenchmarkBot[]; averageLevel: number | null; medianLevel: number | null; averageQuestsCompleted: number | null; level10: number | null };
...
export function medianTimeToLevel(bots: BenchmarkBot[], level: number, levelups: Map<number, Map<number, number>>): number | null { if (level === 1) return bots.length ? 0 : null; return median(bots.flatMap((bot) => { const seconds = levelups.get(bot.guid)?.get(level); return seconds === undefined ? [] : [seconds]; })); }
export function groupStats(bots: BenchmarkBot[], keyFor: (bot: BenchmarkBot) => string, labelFor: (key: string) => string, levelups: Map<number, Map<number, number>>): GroupStat[] { const groups = new Map<string, BenchmarkBot[]>(); for (const bot of bots) { const key = keyFor(bot); groups.set(key, [...(groups.get(key) ?? []), bot]); } return [...groups.entries()].map(([key, groupBots]) => ({ label: labelFor(key), bots: groupBots, averageLevel: average(groupBots.map((bot) => bot.level)), medianLevel: median(groupBots.map((bot) => bot.level)), averageQuestsCompleted: average(groupBots.flatMap((bot) => typeof bot.questsCompletedAmount === "number" ? [bot.questsCompletedAmount] : [])), level10: median(groupBots.flatMap((bot) => { const seconds = levelups.get(bot.guid)?.get(10); return seconds === undefined ? [] : [seconds]; })) })).sort((a, b) => (b.medianLevel ?? 0) - (a.medianLevel ?? 0) || a.label.localeCompare(b.label)); }
