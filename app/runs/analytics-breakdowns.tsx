...
export function AnalyticsBreakdowns({ selected, levelups }: { selected: RunSummary | null; levelups: Map<number, Map<number, number>> }) {
  const latest = selected;
  if (!latest) return null;

  const hasLevelupHistory = latest.bots.some((bot) => (levelups.get(bot.guid)?.size ?? 0) > 0);
  ...
  for (const bot of latest.bots) counts.set(bot.level, (counts.get(bot.level) ?? 0) + 1);
  ...
  return <>
    ...
    {FEATURE_STEP >= 9 && <section style={{ ...sectionStyle, marginBottom: 18 }}><SectionTitle eyebrow="HOW MANY ACTUALLY GOT THERE" title="Milestone reach rate" note="Stops two turbo hunters from claiming the whole cohort is amazing." />{hasLevelupHistory ? <div style={{ display: "grid", gap: 14 }}>{LEVELS.map((level) => { const item = latest.milestones[level]; const percent = item.total ? item.reached / item.total * 100 : 0; return <div key={level}><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}><strong>Level {level}</strong><span style={{ color: "#8fa6a1" }}>{item.reached}/{item.total} · {percent.toFixed(0)}%</span></div><div style={barTrackStyle}><i style={{ ...barFillStyle, width: `${percent}%` }} /></div></div>; })}</div> : <div style={emptyStyle}>No level-up history recorded for this cohort.</div>}</section>}
  </>;
}
